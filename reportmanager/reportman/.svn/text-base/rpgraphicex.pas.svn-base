  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
            Raise Exception.Create('JPeg compression disabled');
...
begin
  ...
  begin
    ...
    if Result then
    begin
      // Small bug in graphic ex first check and after read, wrong
//      Result := (Header.FileID in [$0A, $0C]) and
//                (Header.Version in [0, 2, 3, 5]) and
//                (Header.Encoding in [0, 1]);
//      ReadBuffer(Header, SizeOf(Header));
      // Corrent, first read then check
      ReadBuffer(Header, SizeOf(Header));
      Result := (Header.FileID in [$0A, $0C]) and
                (Header.Version in [0, 2, 3, 5]) and
                (Header.Encoding in [0, 1]);
