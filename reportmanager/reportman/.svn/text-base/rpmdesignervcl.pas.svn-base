...
type
 ...
 TRpDesignerVCL=class(TComponent)
  private
   FFilename:string;
   FReadOnly:boolean;
   FReport:TRpReport;
...
constructor TRpDesignerVCL.Create(AOwner:TComponent);
begin
...
procedure TRpDesignerVCL.LoadFromStream(stream:TStream);
begin
...
procedure TRpDesignerVCL.LoadFromFile(AFilename:string);
begin
 if Assigned(FReport)then
 begin
  FReport.free;
...
procedure TRpDesignerVCL.CheckLoaded;
begin
 // Loads the report
 if Assigned(FReport) then
  exit;
...
function TRpDesignerVCL.Execute:boolean;
var
 handled:boolean;
 stream:TStream;
 dia:TFRpMainfVCL;
...
procedure TRpDesignerVCL.SaveToStream(stream:TStream);
begin
 CheckLoaded;
