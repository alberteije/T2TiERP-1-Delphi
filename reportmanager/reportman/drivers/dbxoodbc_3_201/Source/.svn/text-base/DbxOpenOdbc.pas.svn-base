...
const
  DbxOpenOdbcVersion = '3.201 (2008-09-26): Beta';
...
type
  ...
  protected
    ...
    procedure OdbcCheck(OdbcCode: SqlReturn; const OdbcFunctionName: AnsiString;
      DbxConStmt: PDbxConStmt; TraceCat: TRACECat = trUNKNOWN); {$IFDEF _INLINE_} inline; {$ENDIF}
  ...
  protected
    ...
    procedure OdbcCheck(OdbcCode: SqlReturn; const OdbcFunctionName: AnsiString; eTraceCat: TRACECat = trUNKNOWN); {$IFDEF _INLINE_} inline; {$ENDIF}
...
var
  ...
  // ---
  function GetTraceCatName(): AnsiString;
  begin
    case eTraceCat of
      trQPREPARE: Result := 'prepare';  { prepared query statements }
      trQEXECUTE: Result := 'execute';  { executed query statements }
      trERROR:    Result := '';         { vendor errors }
      trSTMT:     Result := 'stmt';     { statement ops (i.e. allocate, free) }
      trCONNECT:  Result := 'connect';  { connect / disconnect }
      trTRANSACT: Result := 'transact'; { transaction }
      trBLOB:     Result := 'blob i/o'; { blob i/o }
      trMISC:     Result := 'misc';     { misc. }
      trVENDOR:   Result := 'vendor';   { vendor calls }
      trDATAIN:   Result := 'datain';   { parameter bound data }
      trDATAOUT:  Result := 'dataout';  { trace fetched data }
      else { trUNKNOWN: }
                  Result := 'unknown ';
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      pMessageText[BufferLengthRet] := #0;

      CheckCodeText := StrPas(pMessageText);
      if CheckCodeText <> '' then
      begin
        {$IFDEF _DBXCB_}
        if eTraceCat <> trUNKNOWN then
        begin
          if Assigned(Connection) and Assigned(Connection.fDbxTraceCallbackEven) then
            Connection.DbxCallBackSendMsg(trERROR, 'Error (' + GetTraceCatName() + '): '+ CheckCodeText);
          eTraceCat := trUNKNOWN;
        end;
        {$ENDIF}
...
begin
  ...
  if Assigned(fDbxTraceCallbackEven) then
    DbxCallBackSendMsgFmt(trVENDOR, 'Connect: clone: "%s"', [fOdbcConnectStringHidePassword]);
  {$ENDIF}
  with fOwnerDbxDriver.fOdbcApi do
  begin
...
begin
  ...
  if not ((fOwnerDbxDriver.fClientVersion = 30) and (TraceCat = trSTMT)) then { <=: QC: 58678: skip Delphi 2006 callback bag for TSQLMonitor.UpdateTraceCallBack }
...
procedure TSqlConnectionOdbc.DbxCallBackSendMsgFmt;//(TraceCat: TRACECat; const FmtMsg: AnsiString; const FmtArgs: array of const);
var
  aMsg: AnsiString;
begin
  try
...
begin
    ...
    begin
      ...
      if OdbcRetcode <> OdbcApi.SQL_SUCCESS then
        fOwnerDbxDriver.OdbcCheck(OdbcRetcode, 'SQLAllocHandle(SQL_HANDLE_STMT)',
          SQL_HANDLE_STMT, fDbxConStmt.fHCon, @fDbxConStmt, nil, nil, nil, 0, trSTMT);
      {$IFDEF _DBXCB_}
      if Assigned(fDbxTraceCallbackEven) then
        DbxCallBackSendMsgFmt(trSTMT, 'stmt allocate: "$%x"', [Integer(HStmt)]);
      {$ENDIF}
  ...
  begin
    ...
    if OdbcRetcode <> OdbcApi.SQL_SUCCESS then
      fOwnerDbxDriver.OdbcCheck(OdbcRetcode, 'SQLAllocHandle(SQL_HANDLE_STMT)',
        SQL_HANDLE_STMT, fhCon, nil, nil, nil, nil, 0, trSTMT);
    {$IFDEF _DBXCB_}
    if Assigned(fDbxTraceCallbackEven) then
      DbxCallBackSendMsgFmt(trSTMT, 'stmt allocate: "$%x"', [Integer(HStmt)]);
    {$ENDIF}
...
var
  ...
  begin
    ...
    begin
      {$IFDEF _DBXCB_}
      if Assigned(fDbxTraceCallbackEven) then
        DbxCallBackSendMsgFmt(trSTMT, 'stmt free: "$%x"', [Integer(HStmt)]);
      {$ENDIF}
      ...
      if (OdbcRetcode <> OdbcApi.SQL_SUCCESS) and (not iDbxConStmt.fDeadConnection) then
        fOwnerDbxDriver.OdbcCheck(OdbcRetcode, 'SQLFreeHandle(SQL_HANDLE_STMT)',
          SQL_HANDLE_STMT, HStmt, @iDbxConStmt, nil, nil, nil, 0, trSTMT);
    end;
...
begin
  ...
  begin
    ...
    if (OdbcRetcode <> OdbcApi.SQL_SUCCESS) and (not fConnectionClosed) then
      OdbcCheck(OdbcRetcode, 'SQLFreeHandle(SQL_HANDLE_STMT)', nil, trSTMT);
    HStmt := SQL_NULL_HANDLE;
    dec(fSqlHStmtAllocated);
  end;
  {$IFDEF _TRACE_CALLS_}
...
//  const OdbcFunctionName: AnsiString; DbxConStmt: PDbxConStmt; TraceCat: TRACECat = trUNKNOWN);
begin
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(fDbxTraceCallbackEven) then
    DbxCallBackSendMsgFmt(trTRANSACT, 'Begin: constr: "%s"', [fOdbcConnectStringHidePassword]);
  {$ENDIF}
  {
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(fDbxTraceCallbackEven) then
    DbxCallBackSendMsgFmt(trTRANSACT, 'Commit: constr: "%s"', [fOdbcConnectStringHidePassword]);
  {$ENDIF}
  with fOwnerDbxDriver.fOdbcApi do
  try
    if fStatementPerConnection = 0  then
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(fDbxTraceCallbackEven) then
    DbxCallBackSendMsgFmt(trTRANSACT, 'Rollback: constr: "%s"', [fOdbcConnectStringHidePassword]);
  {$ENDIF}
  with fOwnerDbxDriver.fOdbcApi do
  try
    if fStatementPerConnection = 0  then
 ...
 {$ENDIF IFDEF _MIXED_FETCH_}
  ...
  begin
    ...
    begin
      ...
      begin
        ConnectionOptionsValues[iConnectionOption] := #0;
        continue;
      end;
      ...
      if (ConnectionOptionsValues[iConnectionOption] <> #0) and ( not SetConnectionOption(fConnectionOptions,
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(fDbxTraceCallbackEven) then
    DbxCallBackSendMsgFmt(trCONNECT, 'Connect: user "%s", server "%s"', [StrPas(UserName), StrPas(ServerName)]);
  {$ENDIF}
  if fConnected then
    Exit;
  ...
  try
    ...
    {$IFDEF _DBXCB_}
    if Assigned(fDbxTraceCallbackEven) then
      DbxCallBackSendMsgFmt(trVENDOR, 'Connect: prepare: "%s"', [fOdbcConnectStringHidePassword]);
    {$ENDIF}
...
{$ENDIF}
    ...
    {$IFDEF _DBXCB_}
    if Assigned(fDbxTraceCallbackEven) then
      DbxCallBackSendMsgFmt(trVENDOR, 'Connect: returned: "%s"', [fOdbcConnectStringHidePassword]);
    {$ENDIF}
    ...
    if not fSupportsTransaction then
       // clear last error:
       fOwnerDbxDriver.RetrieveOdbcErrorInfo(OdbcRetcode, SQL_HANDLE_DBC, nil, fhCon, Self, nil, nil, 1);
    ...
    *)
    ...
    begin
      ...
      if (tmpS<>#0) and (Pos('\\.\PIPE\', UpperCase(string(tmpS)))>0) then
      begin
...
{$IFDEF _TRACE_CALLS_}
  ...
  LogInfoProc(['25-OdbcDriverLevel =', fOdbcDriverLevel]);
  LogInfoProc(['26-OdbcCatalogPrefix =', fOdbcCatalogPrefix]);
  for i := Low(TConnectionOption) to High(TConnectionOption) do
  begin
    LogInfoProc([AnsiString(IntToStr(27+Byte(i))+'-'+string(cConnectionOptionsNames[i])+' ='),
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(fDbxTraceCallbackEven) then
    DbxCallBackSendMsgFmt(trCONNECT, 'Disconnect: constr: "%s"', [fOdbcConnectStringHidePassword]);
  {$ENDIF}
  with fOwnerDbxDriver.fOdbcApi do
  try
...
var
  aQuotedQualifiedName: AnsiString;
...
//
      ...
      begin
        ...
        if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) then
          fOwnerDbxConnection.DbxCallBackSendMsgFmt(trSTMT, 'stmt close cursor: "$%x"', [Integer(fHStmt)]);
        {$ENDIF}
...
procedure TSqlCommandOdbc.OdbcCheck;//(OdbcCode: SqlReturn; const OdbcFunctionName: AnsiString; eTraceCat: TRACECat = trUNKNOWN);
begin
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) then
    fOwnerDbxConnection.DbxCallBackSendMsg(trQEXECUTE, fSQL);
  {$ENDIF}
...
begin
  ...
  try try
  if bUseUnicodeOdbc then
    LogEnterProc('TSqlCommandOdbc.DoExecuteImmediate', ['SQL =', PWideChar(SQL), 'UseUnicodeOdbc =', bUseUnicodeOdbc])
  else
    LogEnterProc('TSqlCommandOdbc.DoExecuteImmediate', ['SQL =', StrPas(SQL), 'UseUnicodeOdbc =', bUseUnicodeOdbc]);
  {$ENDIF _TRACE_CALLS_}
  Pointer(Cursor) := nil;
  fOdbcRowsAffected := 0;
  with fOwnerDbxDriver.fOdbcApi do
  try
    ...
    begin
      ...
      if (OdbcRetcode <> OdbcApi.SQL_NO_DATA) and (OdbcRetcode <> OdbcApi.SQL_SUCCESS) then
        OdbcCheck(OdbcRetcode, 'SQLExecDirectW', trQEXECUTE);
    end
    else
    begin
      OdbcRetcode := SQLExecDirect(fHStmt, PAnsiChar(fSqlPrepared), SQL_NTS);
      ...
      // update/delete any rows
      if (OdbcRetcode <> OdbcApi.SQL_NO_DATA) and (OdbcRetcode <> OdbcApi.SQL_SUCCESS) then
        OdbcCheck(OdbcRetcode, 'SQLExecDirect', trQEXECUTE);
    end;
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) then
    fOwnerDbxConnection.DbxCallBackSendMsg(trQEXECUTE, StrPas(SQL));
  {$ENDIF}
...
begin
  ...
  try try
  if bUseUnicodeOdbc then
    LogEnterProc('TSqlCommandOdbc.DoPrepare', ['SQL =', PWideChar(SQL), 'ParamCount =', ParamCount,
      'UpdateParams =', UpdateParams, 'UseUnicodeOdbc =', bUseUnicodeOdbc, 'StoredProc =', fStoredProc=1])
  else
    LogEnterProc('TSqlCommandOdbc.DoPrepare', ['SQL =', StrPas(SQL), 'ParamCount =', ParamCount,
      'UpdateParams =', UpdateParams, 'UseUnicodeOdbc =', bUseUnicodeOdbc, 'StoredProc =', fStoredProc=1]);
  {$ENDIF _TRACE_CALLS_}
  if SQL = nil then
  begin
  ...
  try
    ...
    begin
      ...
      {$IFDEF _RegExprParser_}
      fOwnerDbxConnection.fObjectNameParser.DecodeObjectFullName(
       fSql, fCatalogName, fSchemaName, fSql);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            OdbcCheck(OdbcRetcode, 'SQLPrepareW', trQPREPARE);
          end;
        end
        else
        begin
          ...
          if OdbcRetcode <> OdbcApi.SQL_SUCCESS then
            OdbcCheck(OdbcRetcode, 'SQLPrepare', trQPREPARE);
        end;
      end;
      fPreparedOnly := True;
    end
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) then
    fOwnerDbxConnection.DbxCallBackSendMsg(trQPREPARE, StrPas(SQL));
  {$ENDIF}
  if SQL = nil then
  begin
    Result := DBXERR_INVALIDPARAM;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if iPrecision < 256 then
            fOdbcParamCbColDef := 256
          else
            fOdbcParamCbColDef := iPrecision;
          if fBindOutputBufferLength < 256 then
            fBindOutputBufferLength := 256
        end
        else
        begin
          if iPrecision < 255 then
            ...
            fOdbcParamCbColDef := iPrecision;
          if fBindOutputBufferLength < 255 then
            fBindOutputBufferLength := 255
        end;
      end
      else
      begin
...
begin
  ...
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 30 then
    aCursor := TSqlCursorMetaDataColumns3.Create(Self)
  else
  {$ENDIF}
...
begin
  ...
  {$IFDEF _DBX30_}
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 30 then
    aCursor := TSqlCursorMetaDataIndexes3.Create(Self)
  else
  {$ENDIF}
...
begin
  ...
  {$IFDEF _DBX30_}
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 30 then
    aCursor := TSqlCursorMetaDataProcedureParams3.Create(Self)
  else
  {$ENDIF}
...
begin
  ...
  {$IFDEF _DBX30_}
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 30 then
    aCursor := TSqlCursorMetaDataProcedures3.Create(Self)
  else
  {$ENDIF}
...
begin
  Result := DBXERR_NONE;
  {$IFDEF _TRACE_CALLS_} try try LogEnterProc('TSQLMetaDataOdbc.getTables', ['TableName =', TableName, 'TableType =', TableType]); {$ENDIF _TRACE_CALLS_}
  Pointer(Cursor) := nil;
  {$IFDEF _DBX30_}
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 30 then
    aCursor := TSqlCursorMetaDataTables3.Create(Self)
  else
  {$ENDIF}
...
begin
  ...
  begin
    ...
    if (ColNo > 0) and ( not TOdbcBindCol(fOdbcBindList[ColNo - 1]).fIsFetched ) then
...
begin
  ...
  begin
    ...
    {$IFDEF _DBXCB_}
    if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) then
      fOwnerDbxConnection.DbxCallBackSendMsgFmt(trBLOB, 'fetch long column: "%s"', [StrPas(fColName)]);
    {$ENDIF}
...
begin
  ...
  begin
    ...
    IsBlank := (fColValueSizePtr^ = OdbcApi.SQL_NULL_DATA) or
      ...
      {+?}// HyperFile DBMS:
      {
      or (fOdbcHostVarAddress.ptrAnsiChar^ = #0)
...
var
  QuoteChar: AnsiChar;
  // ---
  procedure DefaultParseTableName(TableName: PAnsiChar); {$IFDEF _INLINE_} inline; {$ENDIF}
  var
    dot1, dot2: PAnsiChar;
  ...
  // ---
  procedure InformixParseTableName(TableName: PAnsiChar); {$IFDEF _INLINE_} inline; {$ENDIF}
  var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        StrLCopy(fMetaCatalogName, PAnsiChar(vStr), Length(vStr) + 1);
      end;
    ...
    begin
      ...
      begin
        ...
        StrLCopy(fMetaSchemaName, PAnsiChar(vStr), Length(vStr) + 1);
      end;
      vTable := Copy(vTable, p + 1, Length(vTable) - p);
      if Length(vTable) = 0 then
      begin
    ...
    StrLCopy(fMetaTableName, PAnsiChar(vTable), Length(vTable) + 1);
  end;
  // ---
{$ENDIF}
begin
  ...
  {$IFDEF _TRACE_CALLS_}
    ...
    {$else}
    finally LogExitProc('TSqlCursorMetaData.ParseTableNameBase', ['MetaTableName =', fMetaTableName]); end;
    {$endif}
  {$ENDIF _TRACE_CALLS_}
end;
...
{$IFDEF _RegExprParser_}
  ...
  begin
    ...
    StrLCopy(fQualifiedTableName, PAnsiChar(vObjectName), Length(vObjectName) + 1);
  end
...
{$ELSE}
  if WantCatalog then
    ...
    begin
      ...
      begin
        ...
        StrCopy(fQualifiedTableName, PAnsiChar(QuoteChar + AnsiString(Cat) + QuoteChar
          + '.' + QuoteChar + AnsiString(Schema) + QuoteChar
          + '.' + QuoteChar + AnsiString(TableName) + QuoteChar));
      end
      else
      begin
        GetMem(fQualifiedTableName, aTableLen + aSchemaLen + aCatLen + 3);
        StrCopy(fQualifiedTableName, PAnsiChar(AnsiString(Cat) + '.' + AnsiString(Schema) + '.' +
    ...
    begin
      ...
      begin
        ...
        StrCopy(fQualifiedTableName, PAnsiChar(AnsiString(Cat) + '..' + AnsiString(TableName)));
      end;
    end
  ...
  begin
    ...
    begin
      ...
      StrCopy(fQualifiedTableName, PAnsiChar(QuoteChar + AnsiString(Schema) + QuoteChar
        + '.' + QuoteChar + AnsiString(TableName) + QuoteChar));
    end
    else
    begin
      GetMem(fQualifiedTableName, aTableLen + aSchemaLen + 2);
      StrCopy(fQualifiedTableName, PAnsiChar(AnsiString(Schema) + '.' + AnsiString(TableName)));
    end;
  end
  else
  begin
    ...
    begin
      ...
      StrCopy(fQualifiedTableName, PAnsiChar(QuoteChar + AnsiString(TableName) + QuoteChar));
    end
    else
    begin
...
constructor TSqlCursorMetaDataTables.Create;//(OwnerMetaData: TSQLMetaDataOdbc);
begin
...
begin
  {$IFDEF _TRACE_CALLS_}
    if Value<>nil then PAnsiChar(Value)^ := #0;
    IsBlank := True;
    Result := DBXERR_NONE;
    try try
...
constructor TSqlCursorMetaDataColumns.Create;//(OwnerMetaData: TSQLMetaDataOdbc);
begin
...
constructor TSqlCursorMetaDataProcedures.Create;//(OwnerMetaData: TSQLMetaDataOdbc);
begin
...
constructor TSqlCursorMetaDataProcedureParams.Create;//(OwnerMetaData: TSQLMetaDataOdbc);
begin
...
//  SearchCatalogName,
//  SearchSchemaName,
//  SearchProcedureName: PAnsiChar;
//  SearchParamName: PAnsiChar);
var
  OdbcRetcode: OdbcApi.SqlReturn;
  Cat: PAnsiChar;
  Schema: PAnsiChar;
...
//      v_CHAR_OCTET_LENGTH := 0;
    ...
    begin
      ...
      {$IFDEF _RegExprParser_}
        //sTypeName := UpperCase(fSqlConnectionOdbc.fObjectNameParser.EncodeObjectFullName(fMetaCatalogName,
        ...
        then
          i := 1;
      {$ELSE}
        SearchCatalogName := fMetaCatalogName;
        SearchSchemaName := fMetaSchemaName;
        SearchParamName := fMetaTableName;

        fMetaCatalogName := nil;
        fMetaSchemaName := nil;
        fMetaTableName := nil;

        ParseTableNameBase(PAnsiChar(fSqlConnectionOdbc.fLastStoredProc.fSql));
        if  (StrCompNil(fMetaTableName, SearchParamName) = 0)
          and (StrCompNil(fMetaCatalogName, PAnsiChar(fSqlConnectionOdbc.fLastStoredProc.fCatalogName)) = 0)
          and (StrCompNil(fMetaSchemaName, PAnsiChar(fSqlConnectionOdbc.fLastStoredProc.fSchemaName)) = 0)
        then
          i := 1;
        ...
        ClearMetaData();
        fMetaCatalogName := SearchCatalogName;
        fMetaSchemaName := SearchSchemaName;
        fMetaTableName := SearchParamName;
