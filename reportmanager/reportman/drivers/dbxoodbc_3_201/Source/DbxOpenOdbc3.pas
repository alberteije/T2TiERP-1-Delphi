...
function StringToWideChar(const Source: AnsiString; Dest: PWideChar; DestChars: Integer): Integer; overload; {$IFDEF _INLINE_} inline; {$ENDIF}
begin
...
begin
  ...
  if Assigned(TSqlConnectionOdbc3(fOwnerDbxConnection).fDbxTraceCallbackEven) then
    TSqlConnectionOdbc3(fOwnerDbxConnection).DbxCallBackSendMsg(trQPREPARE, AnsiString(WideString(SQL)));
  {$ENDIF}
  if SQL = nil then
  begin
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(TSqlConnectionOdbc3(fOwnerDbxConnection).fDbxTraceCallbackEven) then
    TSqlConnectionOdbc3(fOwnerDbxConnection).DbxCallBackSendMsg(trQEXECUTE, fSQL);
  {$ENDIF}
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(TSqlConnectionOdbc3(fOwnerDbxConnection).fDbxTraceCallbackEven) then
    TSqlConnectionOdbc3(fOwnerDbxConnection).DbxCallBackSendMsg(trQEXECUTE, AnsiString(WideString(SQL)));
  {$ENDIF}
  if SQL <> nil then
  begin
...
begin
  ...
  begin
    sOptionValue := AnsiString(WideString(PWideChar(TableName)));
...
begin
  Result := DBXERR_NONE;
  {$IFDEF _TRACE_CALLS_} try try LogEnterProc('TSqlCursorMetaDataTables3.getWideString', ['ColumnNumber =', ColumnNumber]); {$ENDIF _TRACE_CALLS_}
  // ansi:
  //
  iMaxChars := 0;
...
begin
  Result := DBXERR_NONE;
  {$IFDEF _TRACE_CALLS_} try try LogEnterProc('TSqlCursorMetaDataColumns3.getWideString', ['ColumnNumber =', ColumnNumber]); {$ENDIF _TRACE_CALLS_}
  // ansi:
  //
  iMaxChars := 0;
  getColumnLength(ColumnNumber, iMaxChars);
...
begin
  Result := DBXERR_NONE;
  {$IFDEF _TRACE_CALLS_} try try LogEnterProc('TSqlCursorMetaDataIndexes3.getWideString', ['ColumnNumber =', ColumnNumber]); {$ENDIF _TRACE_CALLS_}
  // ansi:
  //
  iMaxChars := 0;
  getColumnLength(ColumnNumber, iMaxChars);
...
begin
  Result := DBXERR_NONE;
  {$IFDEF _TRACE_CALLS_} try try LogEnterProc('TSqlCursorMetaDataProcedures3.getWideString', ['ColumnNumber =', ColumnNumber]); {$ENDIF _TRACE_CALLS_}
  // ansi:
  //
  iMaxChars := 0;
  getColumnLength(ColumnNumber, iMaxChars);
...
begin
  Result := DBXERR_NONE;
  {$IFDEF _TRACE_CALLS_} try try LogEnterProc('TSqlCursorMetaDataProcedureParams3.getWideString', ['ColumnNumber =', ColumnNumber]); {$ENDIF _TRACE_CALLS_}
  // ansi:
  //
  iMaxChars := 0;
  getColumnLength(ColumnNumber, iMaxChars);
