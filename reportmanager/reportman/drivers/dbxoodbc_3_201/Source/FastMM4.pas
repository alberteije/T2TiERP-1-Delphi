...
{+}
function DWordToStr(Value: DWORD):ShortString;
var
  sBuff: PAnsiChar;
const
 sFormat: String = '%u';
    ...
    begin
      ...
      begin
        ...
        if LCppObjectTypeName <> nil then
          ...
          LMsgPtr := AppendClassNameToBuffer(LClass, LMsgPtr);
        {+}
        //LClassName := LClass.ClassName + ' Addr: ' +
        //  DWordToStr(DWORD(Pointer(Cardinal(APointer) + SizeOf(TFullDebugBlockHeader))));
        {+.}
      end
      else
      begin
...
{$endif}
  ...
  {+}
  {$ifdef EnableMemoryLeakReporting}
  {$IFDEF WIN32}{.$IFNDEF _MEMLEAKCHECK_}
  if not IsLibrary then
