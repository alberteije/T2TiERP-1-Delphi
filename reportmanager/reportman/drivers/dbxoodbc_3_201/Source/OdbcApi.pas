...
uses
  ...
  {$IFDEF _D12UP_}AnsiStrings,{$ENDIF}
...
{$IFDEF DynamicOdbcImport}
  ...
  private
    ...
    hModuleApi: HMODULE;
    fModuleNameTrac: AnsiString;
    hModuleApiTrac: HMODULE;
    fModuleNameInst: AnsiString;
    ...
    {$ENDIF}
    procedure NotImplemented(const sFuncName: AnsiString;
...
{$IFDEF _iOdbcApi_}
  ...
  private
    ...
    {$IFDEF DynamicOdbcImport}
    procedure CheckAvailableProxy; {$IFDEF _INLINE_} inline; {$ENDIF}
    {$ENDIF}
  { IOdbcApi }
  protected
   // MACROs
...
function LoadOdbcDriverManager(LibraryName: PAnsiChar; UnicodePriority: Boolean = False): TOdbcApiProxy;
...
const
  ...
  //
    SQL_MSSQL_VARIANT = (-150); // map to SQL_WVARCHAR/SQL_VARCHAR (fldWIDESTRING/fldZSTRING)
    SQL_MSSQL_XML     = (-152); // map to SQL_WLONGVARCHAR/SQL_LONGVARCHAR (fldWIDESTRING.fldstWIDEMEMO/fldZSTRING.fldstMEMO)
...
const
{$IFDEF MSWINDOWS}
  ODBC32DLL: AnsiString = 'odbc32.dll';
{$ELSE}
  ODBC32DLL: AnsiString = 'libodbc.so';
{$ENDIF}
...
function LoadOdbcDriverManager(LibraryName: PAnsiChar; UnicodePriority: Boolean): TOdbcApiProxy;
begin
  Result := fOdbcApi;
...
function TOdbcApiProxy.GetModuleName: AnsiString;
begin
  Result := ODBC32DLL;
end;
...
{ TStringHash }
  ...
  public
    ...
    procedure BeginLock;
    procedure EndLock;
    function Add(const Key: AnsiString; Value: TObject): PHashItem;
    procedure Clear(bDestroyValues: Boolean = True);
    procedure Remove(const Key: AnsiString; bDestroyValues: Boolean = True); overload;
    function ValueOf(const Key: AnsiString): TObject; overload;
  end;
...
function TStringHash.Add(const Key: AnsiString; Value: TObject): PHashItem;
var
  Hash: Integer;
  Bucket: PHashItem;
begin
...
procedure TStringHash.Remove(const Key: AnsiString; bDestroyValues: Boolean = True);
var
  P: PHashItem;
  Prev: PPHashItem;
begin
...
function TStringHash.ValueOf(const Key: AnsiString): TObject;
var
  P: PHashItem;
begin
  P := Find(Key)^;
...
function LoadOdbcDriverManager(LibraryName: PAnsiChar; UnicodePriority: Boolean): TOdbcApiProxy;
var
  sLibNames, sLib, sLibKey: AnsiString;
  iPos, iLengthLibNames, iTmp: integer;
  vHModule: HMODULE;
  {$IFNDEF LINUX}
  sBuff, sEnv: AnsiString;
  iLenBuff: integer;
  {$ENDIF}
...
const
  cLibsSeparator: AnsiString = ';';
  cSqlAttrOdbcVerSeparator:AnsiString = ':';

  function ExtractNextLibName: AnsiString; //???{$IFDEF _INLINE_} inline; {$ENDIF}
  var
    i: integer;
  begin
    i := iPos;
  ...
  function GetProcAddressEx(Module: HMODULE; const sProcName: AnsiString; PtrA: PPointer;
    PtrW: PPointer = nil): Boolean; //{$IFDEF _INLINE_} inline; {$ENDIF}
  begin
    PtrA^ := GetProcAddress(Module, PAnsiChar(sProcName));
    if Assigned(PtrW) then
    begin
      PtrW^ := GetProcAddress(Module, PAnsiChar(sProcName+'W'));
      if Assigned(PtrW^) and UnicodePriority and Assigned(PtrA^) then
        PtrA^ := nil;
    end;
...
begin
  ...
  try
    ...
    repeat
      ...
      iTmp := AnsiPos(cSqlAttrOdbcVerSeparator, sLib);
      if (iTmp > 0) and (iTmp < Length(sLib)) then
      begin
        vOdbcDriverLevel := StrToIntDef(string(Copy(sLib, 1, iTmp-1)), -1);
      ...
      // make library cache key
      sLibKey := AnsiString(IntToStr(Integer(UnicodePriority))) + AnsiChar(',')
        + AnsiString(IntToStr(vOdbcDriverLevel)) + AnsiChar(':') + sLib;

      // check api cached
      Result := TOdbcApiProxy(fOdbcApiCache.ValueOf(sLibKey));
      if Result <> nil then
      begin
      ...
      {$IFNDEF LINUX}
       ...
       begin
         ...
         iLenBuff := GetEnvironmentVariableA('PATH', PAnsiChar(sBuff), Length(sBuff));
         if iLenBuff > 0 then
         begin
           SetLength(sBuff, iLenBuff);
           sBuff := LowerCase(sBuff);
           if AnsiPos(sEnv, sBuff) <= 0 then
           begin
             sBuff := sEnv + ';' + sBuff;
             ...
             if (Length(sEnv)>9) and
               ...
               and ( StrToIntDef( string(Copy(sEnv, 6, Length(sEnv)-5-4)), -1) > 0 ) then
             begin
               ...
               begin
                 try
                   ...
                   begin
                     sEnv := AnsiString(ReadString('DLLPATH'));
                     if (sEnv<>'') and (sEnv[Length(sEnv)]='\') then
                       SetLength(sEnv, Length(sEnv)-1);
                     if sEnv <> '' then
                     begin
                       if (AnsiPos(LowerCase(sEnv)+#1, sBuff+#1)=0)
                         and (AnsiPos(LowerCase(sEnv)+';', sBuff+';')=0)
                         and (AnsiPos(LowerCase(sEnv)+'\.', sBuff+'\.')=0)
                       then
                         sBuff := sBuff + ';' + sEnv;
                     end;
             ...
             SetEnvironmentVariableA('PATH', PAnsiChar(sBuff));
           end;
         end;
       end;
      {$ENDIF}
      //SetLastError(0);
      vHModule := SafeLoadLibrary(string(sLib));
      if vHModule <> 0 then
      begin
    ...
    with Result do
    try
      fOdbcApiCache.Add(sLibKey, Result);
      ...
      // ODBC32DLL
        ...
        {begin:fix}
          if // ( not fOwnerDbxDriver.fOdbcApi.SystemODBCManager ) and
            ...
            and
            (AnsiPos(AnsiString(#1+sLib+#1), AnsiString(
              // windows libs:
...
procedure TOdbcApiProxy.NotImplemented(const sFuncName: AnsiString;
  iLevel: Integer = OdbcApi_NotImplemented_OdbcName);
var
  sLib: AnsiString;
begin
  ...
  raise EOdbcApiNotImplemented.Create('Not implemented the ODBC API function "' + string(sFuncName) +
    '" in library "' + string(sLib) +'"' );
end;
...
begin
  ...
  begin
    ...
    wszConnStrIn := WideString(StrPas(PAnsiChar(@szConnStrIn)));
    ...
    begin
      ...
      sConnStrOut := AnsiString(wszConnStrOut);
...
var
  ...
  sCharacterAttribute: AnsiString;
...
begin
  ...
  begin
    ...
    begin
      ...
      sCharacterAttribute := AnsiString(wszCharacterAttribute);
...
begin
  ...
  begin
    //ANSI to Unicode mappings:
    wszCatalogName := WideString(StrPas(PAnsiChar(@szCatalogName)));
      //cbCatalogName := SQL_NTS; // or Length(wszCatalogName) + 1;{???: +1}
    wszSchemaName := WideString(StrPas(PAnsiChar(@szSchemaName)));
      //cbSchemaName := Length(wszSchemaName) + 1;
    wszTableName := WideString(StrPas(PAnsiChar(@szTableName)));
      //cbTableName := Length(wszTableName) + 1;
    wszColumnName := WideString(StrPas(PAnsiChar(@szColumnName)));
...
begin
  ...
  begin
    ...
    begin
      wszCatalogName := WideString(StrPas(CatalogName));
    ...
    begin
      wszSchemaName := WideString(StrPas(SchemaName));
    ...
    begin
      wszTableName := WideString(StrPas(TableName));
    ...
    begin
      wszColumnName := WideString(StrPas(ColumnName));
...
var
  ...
  sServerName, sDescription: AnsiString;
begin
  Result := SQL_ERROR;
  ...
  begin
    ...
    begin
      ...
      sServerName := AnsiString(wszServerName);
    ...
    begin
      ...
      sDescription := AnsiString(wszDescription);
...
begin
  ...
  begin
    ...
    begin
      ...
      sColumnName := AnsiString(wszColumnName);
...
begin
  ...
  begin
    ...
    begin
      wszConnStrIn := WideString(StrPas(szConnStrIn));
...
var
  ...
  sDriverDesc, sDriverAttributes: AnsiString;
begin
  Result := SQL_ERROR;
  ...
  begin
    ...
    begin
      ...
      sDriverDesc := AnsiString(wszDriverDesc);
    ...
    begin
      ...
      sDriverAttributes := AnsiString(wszDriverAttributes);
...
var
  ...
  sSqlState, sMessageText: AnsiString;
begin
  Result := SQL_ERROR;
  ...
  begin
    ...
    begin
      sSqlState := AnsiString(wszSqlState);
      sMessageText := AnsiString(wszMessageText);
...
begin
  ...
  begin
    //ANSI to Unicode mappings:
    wszPkCatalogName := WideString(StrPas(PAnsiChar(@szPkCatalogName)));
    wszPkSchemaName := WideString(StrPas(PAnsiChar(@szPkSchemaName)));
    wszPkTableName := WideString(StrPas(PAnsiChar(@szPkTableName)));
    wszFkCatalogName := WideString(StrPas(PAnsiChar(@szFkCatalogName)));
    wszFkSchemaName := WideString(StrPas(PAnsiChar(@szFkSchemaName)));
    wszFkTableName := WideString(StrPas(PAnsiChar(@szFkTableName)));
...
function TOdbcApiProxy.SQLFreeHandle(HandleType: SqlSmallint;
  ...
  procedure DoSQLFreeHandle(); //???{$IFDEF _INLINE_} inline; {$ENDIF}
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      sValuePtr := AnsiString(wszValuePtr);
  ...
  begin
    ...
    if bHandleWS then
      StrLCopy( PansiChar(ValuePtr), vValuePtr, BufferLength);
  end
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      sCursorName := AnsiString(wszCursorName);
...
begin
  ...
  begin
    ...
    begin
      ...
      sDiagInfo := AnsiString(wszDiagInfo);
...
begin
  ...
  begin
    ...
    begin
      ...
      sInfoValuePtr := AnsiString(wszInfoValuePtr);
...
begin
  ...
  begin
    //ANSI to Unicode mappings:
    wszSqlStrIn := Widestring(StrPas(PAnsiChar(@szSqlStrIn)));
...
begin
  ...
  begin
    ...
    begin
      wszProcName := WideString(StrPas(szProcName));
      szProcName := PAnsiChar(PWideChar(wszProcName));
    end;

    if Assigned(szColumnName) then
    begin
      wszColumnName := WideString(StrPas(szColumnName));
...
begin
  ...
  begin
    ...
    if bHandleWS then
    begin
      wszValuePtr := WideString(StrPas(PAnsiChar(ValuePtr)));
...
begin
  ...
  begin
    ...
    begin
      wszCursorName := WideString(StrPas(CursorName));
...
begin
  ...
  begin
    ...
    begin
      wszValue := WideString(StrPas(PAnsiChar(Value)));
...
begin
  ...
  begin
    ...
    begin
      wszTableType := WideString(StrPas(TableType));
...
function TOdbcApiProxy.TraceOpenLogFile(var _1, _2: WideChar; _3: Integer): Retcode;
begin
  Result := SQL_ERROR;
...
function TOdbcApiProxy.ODBCSetTryWaitValue(dwValue: Integer): LongBool;
begin
  Result := False;
...
function TOdbcApiProxy.SQL_DELETE_RECORD(HStmt: SqlHStmt; irow: SqlUSmallint): SqlReturn;
begin
  Result := SQLSetPos(HStmt, irow, SQL_DELETE, SQL_LOCK_NO_CHANGE);
end;
...
function TOdbcApiIntfImpl.SQL_ADD_RECORD(HStmt: SqlHStmt; irow: SqlUSmallint): SqlReturn;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
...
function TOdbcApiIntfImpl.SQL_DELETE_RECORD(HStmt: SqlHStmt; irow: SqlUSmallint): SqlReturn;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
...
function TOdbcApiIntfImpl.SQL_FUNC_EXISTS(pfExists: PUWord; uwAPI: UWord): SqlInteger;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
...
function TOdbcApiIntfImpl.SQL_LOCK_RECORD(HStmt: SqlHStmt; irow, fLock: SqlUSmallint): SqlReturn;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
...
function TOdbcApiIntfImpl.SQL_POSITION_TO(HStmt: SqlHStmt; irow: SqlUSmallint): SqlReturn;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
...
function TOdbcApiIntfImpl.SQL_REFRESH_RECORD(HStmt: SqlHStmt; irow, fLock: SqlUSmallint): SqlReturn;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
...
function TOdbcApiIntfImpl.SQL_UPDATE_RECORD(HStmt: SqlHStmt; irow: SqlUSmallint): SqlReturn;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
...
function TOdbcApiIntfImpl.SQLCopyDesc(SourceDescHandle, TargetDescHandle: SqlHDesc): SqlReturn;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
...
function TOdbcApiIntfImpl.SQLNumParams(HStmt: SqlHStmt; var pcpar: SqlSmallint): SqlReturn;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
...
function TOdbcApiIntfImpl.SQLParamData(StatementHandle: SqlHStmt; var Value: SqlPointer): SqlReturn;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
