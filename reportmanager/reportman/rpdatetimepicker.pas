...
{$IFDEF USEVARIANTS}
  Variants,
{$ENDIF}
  ComCtrls,dbctrls,db,commctrl;

type
  TMyTimePicker=class(TDateTimePicker)
  published
    property TabOrder;
  end;

  TDBDateTimePicker = class(TMyTimePicker)
  private
...
procedure TDBDateTimePicker.PickerChange(Sender:TObject);
begin
 IsModified:=True;
end;
...
begin
  try
    if ((fdatalink.field<>nil) and NOt (FDatalink.field.isnull and FIsnull)) then
     Updatedata(Self);
    FDataLink.UpdateRecord;
  except
    SetFocus;
