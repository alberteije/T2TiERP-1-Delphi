...
begin
 ...
 if (TrpSection(printitem).SectionType in [rpsecpfooter,rpsecpheader,rpsecgheader,rpsecgfooter]) then
 begin
 ...
 if (TrpSection(printitem).SectionType in [rpsecpfooter]) then
 begin
  lnames.Add(SRpSForcePrint);
  ltypes.Add(SRpSBool);
  lhints.Add('refsection.html');
...
begin
 ...
 begin
  ...
  if (Not (ssshift in Shift)) then
   TFRpObjInspVCL(fobjinsp).ClearMultiSelect;
 ...
 begin
  ...
  if assigned(TFRpObjInspVCL(fobjinsp).Combo) then
  begin
   TFRpObjInspVCL(fobjinsp).Combo.Items.AddObject(asizepos.Name,asizeposint);
